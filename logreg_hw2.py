...
def main():
  ...
  test_out = np.concatenate((np.expand_dims(np.array(range(233),dtype=np.int), axis=1), pred_test_y), axis=1)
  header = np.array([["id", "type"]])
  test_out = np.concatenate((header, test_out))
  np.savetxt('test_predicted2.csv', test_out, fmt='%s', delimiter=',')
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  for xi,yi in zip(X,y):
    logf = logistic(wt.dot(xi))
    ll = ((yi*np.log(logf, out = logf, where = logf > .0000001))+((1-yi)*np.log(1-logf, out = logf, where = logf > .0000001)))
    nll += ll
  nll = nll * -1
  return nll
...
######################################################################
def dummyAugment(X):
  dummy = np.ones((np.shape(X)[0],1))
  aug_X = np.append(X,dummy,axis=1)
  return aug_X
